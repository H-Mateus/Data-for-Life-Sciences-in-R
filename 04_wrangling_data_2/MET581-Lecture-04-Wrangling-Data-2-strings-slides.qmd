...
my_file <- "my_work/cohort_after_s4.rds"

cohort <- if (file.exists(my_file)) {
  read_rds(my_file)
} else {
  # missed the session, or lost the file? start from the provided checkpoint
  read_rds("https://raw.githubusercontent.com/H-Mateus/Data-for-Life-Sciences-in-R/main/data/checkpoints/cohort_after_s4.rds")
}
```
...
write_rds(cohort, "my_work/cohort_after_s5.rds")
```
...
Next session: turning these repairs into functions.
