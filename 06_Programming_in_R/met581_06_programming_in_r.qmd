...
format:
  revealjs:
    ...
    footer: "Data for Life Sciences 1 - Slides and code available [here](https://h-mateus.github.io/Data-for-Life-Sciences-in-R/)"
...
## The cleaning thread {.smaller}

```{r}
#| eval: false
library(tidyverse)

# your own work from last session, if you have it
my_file <- "my_work/cohort_after_s5.rds"

cohort <- if (file.exists(my_file)) {
  read_rds(my_file)
} else {
  # missed the session, or lost the file? start from the provided checkpoint
  read_rds("https://raw.githubusercontent.com/H-Mateus/Data-for-Life-Sciences-in-R/main/data/checkpoints/cohort_after_s5.rds")
}
```

The `if` is there so nobody gets stranded. If you have your own file it is used;
if not, you pick up from a checkpoint and carry on.
...
## Save your progress {.smaller}

You have just repaired something. Write it down — the next session starts from
this file.

```{r}
#| eval: false
dir.create("my_work", showWarnings = FALSE)

write_rds(cohort, "my_work/cohort_after_s6.rds")
```

::: {.callout-tip appearance="simple"}
Keep a note of **what** you changed and **why**, not just the cleaned file. A
cleaned dataset nobody can audit is worth very little, and the 50% report is
marked on the pipeline as much as the result.
:::

Next session: exploratory data analysis, where we find what is still wrong.

## Workshop time! {.center}

-   These slides and the workshop can be found on the website [here](https://h-mateus.github.io/Data-for-Life-Sciences-in-R/):

```{r}
#| eval: false
#| include: false
qr <- qrcode::qr_code("https://h-mateus.github.io/Data-for-Life-Sciences-in-R/")
qrcode::generate_svg(qr, here::here("10_resources/00_images/website_qrcode.svg"))
```
