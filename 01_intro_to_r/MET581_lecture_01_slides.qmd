...
```{r}
# here() finds the project root, then builds paths down from it,
# so this resolves correctly on any machine, from any working directory
fs::path_rel(here::here("data", "pheno.txt"), here::here())
```

-   Run `here::here()` on its own in the console and you will see the *full* absolute path for **your** machine — which is exactly the thing you should never hard-code into a script
